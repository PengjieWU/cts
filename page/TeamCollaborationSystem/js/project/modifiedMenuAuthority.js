...
$(function() {
	...
	var routeUrl = "http://127.0.0.1:8088/teamcollaborationsystem/projectMenuInfo/getProjectUserMenuList.action?projectId="+projectId;
	$.ajax({
		type: "GET", //方法类型
		xhrFields: {
		...
		success: function(result) {
			...
			if(result.code == "0") {
				var userMenuList = result.data;
				var str = "";
				$(userMenuList).each(function(index,item){	
						str+="<tr>";
						str+="<td>";
						str+=item.receiveName;
						str+="</td>";
						str+="<td>";
						str+=item.grantorName;
						str+="</td>";
						...
						str+="<a href=\"#\" onclick=\"withDrawMenu(\'";
						str+=item.projectMenuId;
						str+="\')\"";
...
function withDrawMenu(projectMenuId) {
	
	var routeUrl = "http://127.0.0.1:8088/teamcollaborationsystem/projectMenuInfo/withDrawMenu.action?projectMenuId="+projectMenuId;
	$.ajax({
		type: "GET", //方法类型
		xhrFields: {
			withCredentials: true
...
function grantorMenu() {
	var projectId = GetQueryString("projectId");
	window.location.href = "http://127.0.0.1:8020/TeamCollaborationSystem/grantorMenu.html?projectId="+projectId;
}
